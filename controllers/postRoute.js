...
module.exports = {
  ...
  getPrivatePost:async (req,res)=>{
    ...
    try{
      ...
      if(data.user[0].token===data2.token){
        const {userid,page} = req.body
        const len = await Post.find({from:userid})
        const allpost = await Post.find({from:userid})
                                  .sort({time:-1})
                                  .limit(5)
                                  .skip((page-1)*5)
        res.status(200).json({length:len.length,data:allpost})}
    }catch(err){
        return res.status(401).jsom({message:"invalid token"})
    }
  }
